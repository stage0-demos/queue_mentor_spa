...
- Developer [SPA Standard Prerequisites](https://github.com/stage0-demos/queue/blob/main/DeveloperEdition/standards/spa_standards.md)
...
**Note**: This template uses `@stage0-demos/queue_spa_utils` for reusable components, composables, and utilities. See the [spa_utils README](../spa_utils/README.md) for complete documentation on available components (`AutoSaveField`, `AutoSaveSelect`, `ListPageSearch`), composables (`useResourceList`, `useErrorHandler`, `useRoles`), and utilities (`formatDate`, `validationRules`).
...
This template uses components and composables from `@astage0-demos/queue_spa_utils`:
