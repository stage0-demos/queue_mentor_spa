<template>
  <v-container>
    <v-row>
      <v-col>
        <h1 class="text-h4 mb-4">New Plan</h1>
      </v-col>
    </v-row>
          ...
          <v-card-text>
            <v-form ref="formRef" @submit.prevent="handleSubmit">
              <v-text-field
                ...
                persistent-hint
                required
                data-automation-id="plan-new-name-input"
              />
              ...
              <v-textarea
                ...
                rows="3"
                class="mt-4"
                data-automation-id="plan-new-description-input"
              />
              ...
              <v-select
                ...
                :items="statusOptions"
                class="mt-4"
                data-automation-id="plan-new-status-select"
              />
              ...
              <v-card-actions class="px-0 mt-4">
                <v-btn 
                  @click="router.back()" 
                  variant="text"
                  data-automation-id="plan-new-cancel-button"
                >
                  Cancel
                </v-btn>
                <v-spacer />
                <v-btn
                  ...
                  :loading="isPending"
                  :disabled="isPending"
                  data-automation-id="plan-new-submit-button"
                >
                  Create Plan
                </v-btn>
              </v-card-actions>
...
import type { PlanInput } from '@/api/types'
...
const { mutate: createPlan, isPending } = useMutation<{ _id: string }, Error, PlanInput>({
